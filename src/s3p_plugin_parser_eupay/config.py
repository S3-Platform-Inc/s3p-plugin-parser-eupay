...
config = PluginConfig(
    plugin=CoreConfig(
        reference='eupay',         # уникальное имя источника
        type=SOURCE,                            # Тип источника (SOURCE, ML, PIPELINE)
        files=['eupay.py', ],        # Список файлов, которые будут использоваться в плагине (эти файлы будут сохраняться в платформе)
    ...
    payload=payload.PayloadConfig(
        file='eupay.py',                 # python файл плагина (точка входа). Этот файл должен быть указан в `plugin.files[*]`
        classname='EUPay',               # имя python класса в указанном файле
